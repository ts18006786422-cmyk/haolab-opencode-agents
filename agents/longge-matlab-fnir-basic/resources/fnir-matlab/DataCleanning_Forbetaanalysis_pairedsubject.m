...
group = [group;group];    % 因为是两个被试的数据打包了，这里要拆开，最简单的就是把编号重复一下，后续把36:70通道的数据放到1：35数据下面，形成190的被试编号
...
out_folder = 'D:\Research data\Study_Cliff illusion\20260103神经数据（悦悦）\output_beta/';     % 保存路径
...
for c1 = 1:singlebrain
        ...
        x1 = squeeze(data3(:,c1,numE));    % 95 x 70
        x2 = squeeze(data3(:,c1+35,numE));
        x = [x1;x2];
        ...
        beta_clean(:,c1,numE) = x_clean(1:nSubj);
        beta_clean(:,c1+35,numE) = x_clean(nSubj+1:nSubj*2);
end
end
disp('极端值处理完成！');
